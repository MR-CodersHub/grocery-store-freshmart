...
const FreshMartBlog = (() => {
  ...
  function renderBlogCards(posts, container) {
    const base = FreshMartNavbar.getBasePath();
    if (!container) return;
    if (posts.length === 0) {
    ...
    container.innerHTML = posts.map(post => `
      <article class="blog-card bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700">
        <a href="${base}public/pages/blog-details.html?id=${post.id}" class="block">
        ...
        <div class="p-5">
          ...
          <div class="mt-4 flex items-center gap-3">
            <div class="w-8 h-8 rounded-full bg-brand-200 dark:bg-brand-800"></div>
            <div>
